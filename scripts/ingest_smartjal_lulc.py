...
def ingest_lulc(batch_size: int = 200):
    base_path = 'd:/Smart Jal/backend/database/SmartJal'
    shp_path = os.path.join(base_path, 'LULC_Krishna/LULC_Krishna1.shp')
    
    if not os.path.exists(shp_path):
        print(f"❌ Error: Shapefile not found at {shp_path}")
        return

    print(f"📖 Reading LULC shapefile: {shp_path}...")
    ...
    print(f"✅ Loaded {len(gdf)} LULC zones.")
    ...
    for b in range(num_batches):
        ...
        for _, row in chunk.iterrows():
            ...
            batch_data.append({
                "district": "Krishna",
                "grid_code": int(row.get('gridcode', 0)),
        ...
        for attempt in range(max_retries):
            try:
                supabase.table("land_use_zones").insert(batch_data).execute()
